...
exports.pagination = (model) => {
  return async (req, res, next) => {
    ...
    try {
      if (!sortObject) {
        result.paginate = await model
          .find({ email: { $regex: new RegExp(searchTerm, "i") } })
          .skip(startIndex)
          .limit(limit)
          .exec();
      ...
      if (user) {
        ...
        main().catch((err) => {
          res
            ...
            .json({
              message:
                "You need to provide your gmail google for conducting this feature",
            });
        });
      }
    });
...
exports.sendEmailCardPayment = (req, res, next) => {
  let { time, date, emailUser, product } = req.body;
  let output = `
    <div>
    ...
    <div>
    <img src="https://fes-server-backend.s3.ap-southeast-1.amazonaws.com/thankyou.jpg"/>
    </div>
    `;
  User.findOne({ email: emailUser }).exec((err, user) => {
    if (err) {
