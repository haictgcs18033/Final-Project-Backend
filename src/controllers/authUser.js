        ...
        .then(async user => {
            if (user) {
                if (await user.authenticate(req.body.currentPassword) === false) {
                    ...
                    User.updateOne({ _id: req.user._id }, { $set: { hash_password: new_hash_password } }).exec()
                        .then(result => {
...
exports.loginGoogle = (req, res) => {
    ...
    client.verifyIdToken(
        {
            idToken: tokenId,
            audience: '1052309418279-37lkf36gbep0pi8sp9ke0a8uqgl8fogq.apps.googleusercontent.com'
        }
    ).then(result => {
    ...
    User.findOne({email:email}).exec((err,user)=>{
        ...
        if(user){
            ...
            <div>
               ...
               <p>https://final-project-lemon.vercel.app/reset-password/${token}</p>
            </div>
            `
            User.findOne({ email: email }).exec((err, user) => {
                if (err) {
